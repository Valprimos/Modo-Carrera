...
export function Dashboard() {
  ...
  useEffect(() => {
    ...
    gameApi.db.query<LeagueRow[]>('leagues', { dbFileName: activeSave.dbFileName }).then((ls) => {
      setLeagues(ls);
      if (ls[0]) setSelectedLeague(ls[0].id);
    });
    gameApi.db.query<number>('playerCount', { dbFileName: activeSave.dbFileName }).then(setPlayerCount);
  }, [activeSave]);

  useEffect(() => {
    if (!activeSave || !selectedLeague) return;
    gameApi.db.query<ClubRow[]>('clubsInLeague', {
  ...
  useEffect(() => {
    ...
    gameApi.db.query<PlayerRow[]>('squad', {
      dbFileName: activeSave.dbFileName,
